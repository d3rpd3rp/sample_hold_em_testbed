...
def return_verbose_results(hole_cards, winner_list, result_histograms):
    float_iterations = float(sum(winner_list))
    winning_percentages = []
    for index, hole_card in enumerate(hole_cards):
        winning_percentage = float(winner_list[index + 1]) / float_iterations
        if hole_card == (None, None):
            ...
            winning_percentages.append([hole_card, ": ", winning_percentage])
    ties = ["Ties: ", float(winner_list[0]) / float_iterations]
    player_histograms = {}
    for player_index, histogram in enumerate(result_histograms):
        player_histograms[("Player" + str(player_index + 1))] = []
        for index, elem in enumerate(histogram):
            player_histograms[("Player" + str(player_index + 1))].append([hand_rankings[index], ": ", float(elem) / float_iterations])
    return winning_percentages, player_histograms
